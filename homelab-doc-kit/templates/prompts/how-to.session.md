...
# Session — Phase 3: How to (Build / Run Docs) for {{LAB_NAME}}
...
1. **Tree.** "Here's the standard topic-grouped layout (README, overview, platform, network, planning, per-service). Want this structure or a simpler one?" 
2. **Environment map.** From the scoped plan, build the environment/service/dependency topology with real subnets/hardware (or planned ones). Start from [../overview/lab-map.template.md](../overview/lab-map.template.md).
...
- Mirror the convention: `type:` frontmatter on docs, tables for maps, ADRs as numbered files.

## Output
A documentation tree (files or a written plan for them). Confirm structure before writing to disk.

## Handoff
→ Run [../prompts/fit.session.md](../prompts/fit.session.md). These docs are the "Current/Plan State" column.
